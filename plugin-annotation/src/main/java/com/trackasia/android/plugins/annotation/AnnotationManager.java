...
public abstract class AnnotationManager<
    ...
    protected final TrackAsiaMap trackasiaMap;
    ...
    @UiThread
    protected AnnotationManager(MapView mapView, final TrackAsiaMap trackasiaMap, Style style,
        ...
        trackasiaMap.addOnMapClickListener(mapClickResolver = new MapClickResolver());
        trackasiaMap.addOnMapLongClickListener(mapClickResolver);
    ...
    public void onDestroy() {
        trackasiaMap.removeOnMapClickListener(mapClickResolver);
        trackasiaMap.removeOnMapLongClickListener(mapClickResolver);
     ...
     */
    private class MapClickResolver implements TrackAsiaMap.OnMapClickListener, TrackAsiaMap.OnMapLongClickListener {
    ...
    private T queryMapForFeatures(@NonNull LatLng point) {
        return queryMapForFeatures(trackasiaMap.getProjection().toScreenLocation(point));
    }

    @Nullable
    T queryMapForFeatures(@NonNull PointF point) {
        List<Feature> features = trackasiaMap.queryRenderedFeatures(point, coreElementProvider.getLayerId());
        if (!features.isEmpty()) {
