...
import com.trackasia.android.gestures.MoveDistancesObject;
...
final class DraggableAnnotationController {
    ...
    public static DraggableAnnotationController getInstance(MapView mapView, TrackAsiaMap trackasiaMap) {
        if (INSTANCE == null || INSTANCE.mapView != mapView || INSTANCE.trackasiaMap != trackasiaMap) {
            INSTANCE = new DraggableAnnotationController(mapView, trackasiaMap);
        }
        return INSTANCE;
    }
    ...
    DraggableAnnotationController(MapView mapView, TrackAsiaMap trackasiaMap) {
        this(mapView, trackasiaMap, new AndroidGesturesManager(mapView.getContext(), false),
            mapView.getScrollX(), mapView.getScrollY(), mapView.getMeasuredWidth(), mapView.getMeasuredHeight());
    }

    @VisibleForTesting
    public DraggableAnnotationController(MapView mapView, TrackAsiaMap trackasiaMap,
    ...
    boolean onMove(MoveGestureDetector detector) {
        ...
        if (draggedAnnotation != null) {
            ...
            Geometry shiftedGeometry = draggedAnnotation.getOffsetGeometry(
                trackasiaMap.getProjection(), moveObject, touchAreaShiftX, touchAreaShiftY
            );
