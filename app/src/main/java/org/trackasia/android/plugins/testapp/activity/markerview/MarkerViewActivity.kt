package org.trackasia.android.plugins.testapp.activity.markerview
...
import org.trackasia.android.plugins.markerview.MarkerView
import org.trackasia.android.plugins.markerview.MarkerViewManager
...
class MarkerViewActivity :
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mapView.getMapAsync { trackasiaMap ->
            trackasiaMap.setStyle(Style.getPredefinedStyle("Streets")) { _ ->
                findViewById<View>(R.id.fabStyles).setOnClickListener { trackasiaMap.setStyle(Utils.nextStyle) }

                trackasiaMap.moveCamera(CameraUpdateFactory.zoomTo(2.0))

                markerViewManager =
                    MarkerViewManager(mapView, trackasiaMap)
                createCustomMarker()
                createRandomMarkers()

                trackasiaMap.addOnMapLongClickListener(this@MarkerViewActivity)
                trackasiaMap.addOnMapClickListener(this@MarkerViewActivity)
            }
        }
    }
